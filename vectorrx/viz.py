...
def match_structure(original_list, list_of_lists):
    ...
    for i, sublist in enumerate(list_of_lists):
        new_list.extend([original_list[i]] * len(sublist))
    return new_list


def get_cluster_assignments(xb, k=24):
...
def plot_moa_tsne():
    ...
    drug_brand_names = match_structure(search_engine.drug_df['brand_name'].tolist(), search_engine.moa_summaries)
    drug_moa = match_structure(search_engine.drug_df['pharm_class_moa'].fillna('').tolist(), search_engine.moa_summaries)
    drug_pe = match_structure(search_engine.drug_df['pharm_class_pe'].fillna('').tolist(), search_engine.moa_summaries)
    drug_cs = match_structure(search_engine.drug_df['pharm_class_cs'].fillna('').tolist(), search_engine.moa_summaries)
    drug_epc = match_structure(search_engine.drug_df['pharm_class_epc'].fillna('').tolist(), search_engine.moa_summaries)

    drug_brand_names = [d.upper() for d in drug_brand_names]

    # Returns the original L2 normalized embeddings
    xb = search_engine.moa_index.reconstruct_n()
    ...
    colors = [color_map[label[0]] for label in get_cluster_assignments(xb)]  # uses K-means clustering
    ...
    plot_subtitle = 'Color represents unsupervised K-means clustering of the embeddings using k=24'
